...
- **Background-task notifications** (#78) — responses generated from background scheduled triggers (IO-level schedules + per-squad schedules) now surface as visible notifications instead of dying silently in the message stream. A new central dispatcher (`src/notify.ts`) persists every meaningful background response to a new `background_notifications` SQLite table and fans it out to Telegram (via `sendBackgroundNotification` → `sendProactiveMessage` chunking), the TUI (a `╭─🔔 …╰─` block printed above the prompt with the user's in-progress buffer preserved), and any connected SSE client (`{type:"notification", ...}`). A `"meaningful"` heuristic (≥20 chars + heartbeat-phrase denylist for "no active tasks" / "nothing to report" / "all clear" / "idle" / "ok" etc.) suppresses routine heartbeat ticks; configurable via three new flags `backgroundNotifyMode` (`"all" | "meaningful" | "off"`, default `"meaningful"`), `backgroundNotifyTelegram` (default `true`), `backgroundNotifyTui` (default `true`). New REST endpoints `GET /api/notifications`, `POST /api/notifications/:id/read`, `POST /api/notifications/read-all`. 56 unit tests cover the heuristic, dispatcher routing, error resilience, per-channel toggles, and the full store CRUD surface (`node --test` harness wired into the repo for the first time).
