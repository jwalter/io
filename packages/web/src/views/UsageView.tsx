...
function StatCard({ label, value, sub, accent }: { label: string; value: string; sub?: string; accent?: boolean }) {
	return (
		<div className="glass-card border border-white/[0.07] rounded-2xl px-5 py-4">
			...
			<p className={`text-xl font-mono font-medium ${accent ? 'text-[#E43A9C]' : 'text-zinc-100'}`}>{value}</p>
...
function SummaryTab({ records, totals }: { records: UsageEntry[]; totals: UsageResponse['totals'] }) {
	...
	return (
		<div className="space-y-5">
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
				<StatCard label="Total Tokens" value={fmt(totalTokens)} sub={`${fmt(totals.totalInputTokens)} in · ${fmt(totals.totalOutputTokens)} out`} />
				...
				<StatCard label="Avg Cost / Call" value={totals.callCount > 0 ? fmtCost(totals.totalCostUsd / totals.callCount) : '$0.00'} sub={`over ${totals.callCount} calls`} />
			</div>
			...
			<div className="grid grid-cols-1 lg:grid-cols-2 gap-3">
				<div className="glass-card border border-white/[0.07] rounded-2xl p-4">
					...
					{barData.length > 0 ? (
						...
						<div className="h-[180px] flex items-center justify-center text-zinc-700 text-[11px] font-mono">No data</div>
					)}
				</div>
					...
					<thead>
						<tr className="border-b border-white/[0.06] bg-[#1a1a1a]">
							{['Entity', 'Input', 'Output', 'Calls', 'Cost'].map((h) => (
								<th key={h} className={`text-left px-4 py-2.5 text-zinc-600 font-medium ${h === 'Cost' ? 'text-right' : ''}`}>{h}</th>
							))}
						</tr>
					</thead>
					<tbody>
						{entities.map((e) => (
							<tr key={e.name} className="border-b border-white/[0.04] hover:bg-white/[0.015] transition-colors">
						...
						<tr className="bg-[#1a1a1a]">
							...
							<td className="px-4 py-3 text-right text-[#E43A9C] font-medium">{fmtCost(totals.totalCostUsd)}</td>
						</tr>
					</tbody>
				</table>
			</div>
	...
	return (
		<div className="space-y-2">
			...
			{squads.map((sq) => {
				...
				return (
					<div key={sq.id} className="glass-card border border-white/[0.07] rounded-2xl overflow-hidden">
						<button type="button" onClick={() => toggle(sq.id)} className="w-full flex items-center gap-3 px-4 py-3.5 hover:bg-white/[0.02] transition-colors text-left cursor-pointer">
							...
							<div className="flex items-center gap-6 text-[11px] font-mono">
								<span className="text-zinc-500 hidden sm:block"><span className="text-zinc-700 mr-1">in</span>{fmt(sq.inputTokens)}</span>
								<span className="text-zinc-500 hidden sm:block"><span className="text-zinc-700 mr-1">out</span>{fmt(sq.outputTokens)}</span>
								<span className="text-zinc-500"><span className="text-zinc-700 mr-1">calls</span>{sq.calls}</span>
								<span className="text-[#E43A9C] font-medium w-16 text-right">{fmtCost(sq.cost)}</span>
							</div>
						</button>
									...
									<thead>
										<tr className="bg-[#191919]">
											{['Agent', 'Model', 'Input', 'Output', 'Calls', 'Cost'].map((h) => (
												<th key={h} className={`text-left px-4 py-2 text-zinc-700 font-medium ${h === 'Cost' ? 'text-right' : ''}`}>{h}</th>
											))}
										</tr>
									</thead>
									<tbody>
										{agents.map((a) => (
											<tr key={a.role} className="border-t border-white/[0.04] hover:bg-white/[0.015] transition-colors">
	...
	return (
		<div className="overflow-auto rounded-2xl border border-white/[0.07] glass-card">
			<table className="w-full text-[11px] font-mono">
				<thead>
					<tr className="border-b border-white/[0.06] bg-[#1a1a1a]">
						{['Agent', 'Squad', 'Model', 'Input', 'Output', 'Calls', 'Cost', '% of total'].map((h) => (
							<th key={h} className={`text-left px-4 py-2.5 text-zinc-600 font-medium ${h === 'Cost' ? 'text-right' : ''}`}>{h}</th>
						))}
					</tr>
				</thead>
				<tbody>
					{agents.map((a) => (
						<tr key={`${a.squad}:${a.name}`} className="border-b border-white/[0.04] hover:bg-white/[0.015] transition-colors">
							...
							<td className="px-4 py-3">
								<div className="flex items-center gap-2">
									<div className="w-16 h-1.5 rounded-full bg-[#252525] overflow-hidden">
										<div className="h-full bg-[#E43A9C] rounded-full" style={{ width: `${totalCost > 0 ? (a.cost / totalCost) * 100 : 0}%` }} />
									</div>
									<span className="text-zinc-600 text-[10px]">{totalCost > 0 ? `${((a.cost / totalCost) * 100).toFixed(1)}%` : '0%'}</span>
								</div>
							</td>
						</tr>
					))}
...
function IOTab({ records }: { records: UsageEntry[] }) {
	...
	return (
		<div className="space-y-5">
			<div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
				<StatCard label="IO Tokens" value={fmt(totalInput + totalOutput)} sub={`${fmt(totalInput)} in · ${fmt(totalOutput)} out`} />
			...
			<div className="overflow-auto rounded-2xl border border-white/[0.07] glass-card">
				<table className="w-full text-[11px] font-mono">
					...
					<tbody>
						{byRole.map((r) => (
							<tr key={r.name} className="border-b border-white/[0.04] hover:bg-white/[0.015] transition-colors">
...
function TimelineTab({ records }: { records: UsageEntry[] }) {
	...
	const dailyData = useMemo(() => {
		...
		for (const r of records) {
			...
			const existing = byDay.get(date) ?? { date, inputTokens: 0, outputTokens: 0, cost: 0, calls: 0 };
			existing.inputTokens += r.inputTokens;
			existing.outputTokens += r.outputTokens;
			existing.cost += r.estimatedCostUsd ?? 0;
			existing.calls += 1;
		...
		<div className="space-y-5">
			<div className="glass-card border border-white/[0.07] rounded-2xl p-5">
				...
				{chartData.length > 0 ? (
					...
					<div className="h-[220px] flex items-center justify-center text-zinc-700 text-[11px] font-mono">No data</div>
				)}
			</div>
			...
			<div className="overflow-auto rounded-2xl border border-white/[0.07] glass-card">
				<table className="w-full text-[11px] font-mono">
					...
					<tbody>
						{dailyData.map((d) => (
							<tr key={d.date} className="border-b border-white/[0.04] hover:bg-white/[0.015] transition-colors">
...
export function UsageView() {
	...
	if (!data) {
		return <div className="h-full flex items-center justify-center text-zinc-600 font-mono text-[11px]">Loading...</div>;
	}
	...
	return (
		<div className="flex-1 overflow-y-auto p-6">
			<div className="flex items-start justify-between mb-5 gap-4 flex-wrap">
				<div>
					<h2 className="text-2xl tracking-wide text-zinc-100" style={{ fontFamily: "'Bebas Neue', sans-serif" }}>Usage</h2>
